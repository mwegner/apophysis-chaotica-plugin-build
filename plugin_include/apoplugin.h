...
// so these are real weird in jwildfire--looks like they're just arbitrary constants that the generators set along the way
// see i.e. https://github.com/thargor6/JWildfire/blob/master/src/org/jwildfire/create/tina/randomflame/MachineRandomFlameGenerator.java
// I'm not aware of any easy way to save these into the xml per-form without also modifying jwildfire, so
// just putting here to compile things for now with easy replacement later if needed
#define XFORM_COEFF_00 0.82496229 // a
#define XFORM_COEFF_10 0.55902149 // b
#define XFORM_COEFF_20 0.05455807 // e
#define XFORM_COEFF_01 -0.55902149 // c
#define XFORM_COEFF_11 0.82496229 // d
#define XFORM_COEFF_21 0.73105973 // f
...
inline double limitVal(double pValue, double pMin, double pMax) {
	if (pValue < pMin) {
		return pMin;
	} else if (pValue > pMax) {
		return pMax;
...
inline int limitIntVal(int pValue, int pMin, int pMax) {
	if (pValue < pMin) {
		return pMin;
	} else if (pValue > pMax) {
		return pMax;
